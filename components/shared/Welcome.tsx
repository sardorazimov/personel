...
import React, { useId } from "react";
 ...
 const Welcome = () => {
  return (
    <main className="flex relative max-w-screen-xl">
     <div className="h-full  pointer-events-none z-0">
       
      <div className=" h-full  pointer-events-none [mask-image:radial-gradient(ellipse_at_center,transparent,white)]" />
      {Array.from({ length: 6 }).map((_, index) => (
        <div className="flex max-w-screen-xl" key={"grid-column" + index}>
  ...
  return (
    <div className="flex flex-col items-start justify-center  w-60">
      <div className="flex items-center justify-center">
        <Dot  />
        <SVG  className="max-w-screen-2xl"/>
        {/* <Dot /> */}
      </div>
      <SVGVertical className="ml-3 max-w-screen-xl" />
    </div>
  );
};
